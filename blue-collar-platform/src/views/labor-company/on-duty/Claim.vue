<template>
  <div class="claim-management">
    ...
    >
      ...
      <template #column-insuranceType="{ row }">
...
import { Search, Refresh, Plus, Download, View, Edit, Link, Delete, ArrowDown, Upload, Document } from '@element-plus/icons-vue'
...
// 新增
const handleAdd = () => {
  router.push('/tenant/on-duty/claim/form')
}

// 编辑
const handleEdit = (row: ClaimRecord) => {
  router.push(`/tenant/on-duty/claim/form/${row.id}`)
}
...
const handleView = (row: ClaimRecord) => {
  router.push(`/tenant/on-duty/claim/detail/${row.id}`)
}
