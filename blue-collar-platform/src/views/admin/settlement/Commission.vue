<template>
  <div class="admin-commission-page">
    <!-- 查询条件区域 -->
    ...
    <el-card class="table-card">
      ...
      >
        <template #column-commissionAmount="{ row }">
          <span class="amount">¥{{ row.commissionAmount }}</span>
        </template>
...
// 表格ID
const tableId = 'admin-commission-payment'

// 查询参数
const queryParams = ref<CommissionPaymentQueryParams>({
  page: 1,
  pageSize: 10,
  keyword: '',
  status: '',
...
// 表格数据
const tableData = ref<CommissionPayment[]>([])
const total = ref(0)
const loading = ref(false)
const selectedRows = ref<CommissionPayment[]>([])
...
const loadData = async () => {
  try {
    loading.value = true
    const res = await getCommissionPaymentList(queryParams.value)
    if (res.code === 200) {
...
// 选择变化
const handleSelectionChange = (selection: CommissionPayment[]) => {
...
// 发放佣金
const handlePay = async (row: CommissionPayment) => {
  try {
    await ElMessageBox.confirm(
      `确认发放佣金 ¥${row.commissionAmount}?`,
      '提示',
      {
    ...
    paying.value = true
    const res = await payCommission({ ids: [row.id], remark: '' })
    if (res.code === 200) {
...
const handleBatchPay = () => {
  const totalAmount = selectedRows.value.reduce((sum, item) => sum + item.commissionAmount, 0)
  payForm.value = {
...
const handleConfirmPay = async () => {
  try {
    paying.value = true
    const res = await batchPayCommission({
      ids: payForm.value.ids,
      remark: payForm.value.remark
    })
    if (res.code === 200) {
      ElMessage.success('批量佣金发放成功')
